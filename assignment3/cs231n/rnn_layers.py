...
def affine_forward(x, w, b):
    ...
    - cache: (x, w, b)
    """
    out = x.reshape(x.shape[0], -1).dot(w) + b  # [N, D] * [D, M] = [N, M]
    cache = (x, w, b)
    return out, cache
...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    cache = (x, prev_h, Wx, Wh, b, next_h)  # store values for backpropagation
...
def word_embedding_forward(x, W):
    ...
    # out = [[[1,2,3], [10,11,12], [4,5,6], [7,8,9]], [[7,8,9], [4,5,6], [1,2,3], [10,11,12]]]
    out = W[x]
    cache = (x, W)
    pass
...
"""在每个时间步，我们使用仿射函数将该时间步的RNN隐藏向量转换为词汇表中每个单词的分数。这非常类似于在Assignment2 中的仿射层"""
