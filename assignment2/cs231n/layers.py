...
def relu_backward(dout, cache):
    ...
    # 使用元素级乘法，当x>0时结果为1，否则为0，与dout相乘即为所需的dx
    dx = dout * (x > 0)
...
def softmax_loss(x, y):
    ...
    pij = scores / np.sum(scores, axis=1, keepdims=True)
...
def batchnorm_forward(x, gamma, beta, bn_param: map):
    ...
    if mode == "train":
        ...
        # axis = 0 按列求，计算这个 batch 中每个特征的均值和方差
        ...
        # 使用 running_mean 和 running_var 对测试数据进行归一化
...
def batchnorm_backward(dout, cache):
    ...
    dx_norm = dout * gamma
    # 求 dl/dvar
    dvar = np.sum(dx_norm * (x - mean) * (-0.5) * (var + eps) ** -(3 / 2), axis=0)
    # 求 dl/dmean
    dmean = np.sum(dx_norm * -1 / np.sqrt(var + eps), axis=0) + dvar * np.mean(
        -2 * (x - mean), axis=0
    )
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    mean = np.mean(x, axis=1, keepdims=True)  # 求每个样本的均值
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        #######################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # 生成一个与x相同形状的随机矩阵，元素范围在[0, 1]之间，如果大于p则为0，否则为1
            ...
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dx[
                        n,
                        c,
                        i * stride + max_index[0],
                        j * stride + max_index[1],
                    ] += dout[n, c, i, j]
    pass
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx = (
        D * dx_norm_group
        - np.sum(dx_norm_group, axis=(2, 3, 4), keepdims=True)
        - x_norm_group
        * np.sum(dx_norm_group * x_norm_group, axis=(2, 3, 4), keepdims=True)
    )
    dx /= D * np.sqrt(var + eps)
